...
서버에서 푸시 메시지를 받으면 Analytics SDK는 Notification Bar에 알림을 표시합니다. 이 알림을 터치하면 앱을 실햅합니다. 이때 아무런 설정을 하지 않으면 해당 앱의 Main Activity를 띄웁니다.
...
다음으로 Toast Cloud Push SDK에서 제공하는 register 메서드에서 인자로 설정했던 사용자 ID 정보를 GameAnalytics.tracePushToken 메서드의 인자로 설정하여 호출해 주어야 합니다. register 메서드에 대한 자세한 내용은 Toast Cloud Push에서 제공하는 “Client SDK Guide” 문서를 참고하세요. 
    ...
    int error = PushSdk.register(APP_KEY, UID, new PushSdk.OnRegister() {
        ...
        public void fire(int error) {
            if (error == 0) {
                ...
                GameAnalytics.tracePushToken(UID); 
            } 
        }
    }
……
