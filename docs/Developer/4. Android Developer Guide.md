...
google play service 버전은 환경에따라 다를 수 있습니다. Toast Analytics는 google play service 버전에 특별한 의존성은 없으니 각 프로젝트에 맞게 적절한 버전을 사용하면 됩니다.
...
### Proguard 설정
Unity3D를 사용하지 않는 경우 Unity3D 관련 jar파일이 없어서 빌드중에 proguard에서 에러가 발생할 수 있습니다.

proguard 설정에 '-dontwarn com.unity3d.player.**'를 추가하면 됩니다.

해당 class는 Unity3D를 사용하는 경우에만 참조하기 때문에 동작에는 아무런 문제가 없습니다.
...
다음으로 Toast Cloud Push SDK에서 제공하는 register 메서드에서 인자로 설정했던 사용자 ID 정보를 GameAnalytics.tracePushUserId 메서드의 인자로 설정하여 호출해 주어야 합니다. 이 사용자 ID 정보는 GameAnalytics.setUserId 에서 설정한 사용자 ID와는 다를수 있으며, 반드시 register 메서드의 인자로 사용한 사용자 ID 정보여야 합니다. register 메서드에 대한 자세한 내용은 Toast Cloud Push에서 제공하는 “Client SDK Guide” 문서를 참고하세요. 
...
현재 확인 가능한 값은 Device ID, Campaign User ID, Push User ID입니다. 이 값들은 캠페인 연동 테스트시 필요합니다. 자세한 내용은 “캠페인 연동 가이드”를 참고하세요. Push User ID는 푸시 기능을 위해 Toast Cloud Push SDK를 사용하는 경우의 유저 아이디 정보입니다.
